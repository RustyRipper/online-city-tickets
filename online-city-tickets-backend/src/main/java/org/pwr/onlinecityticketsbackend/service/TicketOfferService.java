...
public class TicketOfferService {
    ...
    // NOTE: In this method we use existsById before findById due to the strange behavior of the findById method,
    // which returns deleted entities, which should not be the case, this is probably a bug in Hibernate,
    // as there is no information in the documentation that findById should return soft deleted entities.
    // It is also possible that this bug only occurs in a test environment, which would require further testing.
